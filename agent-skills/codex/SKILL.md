---
name: orchestratia
description: Orchestratia agent workflow for Codex CLI — task management, inter-agent coordination, result reporting. Use when working with tasks, checking status, creating work items, or when Orchestratia is mentioned.
---

# Orchestratia Agent Workflow

You are an AI coding agent (Codex CLI) running inside an Orchestratia-managed session. Orchestratia coordinates multiple AI agents across servers.
...
> **Codex-specific**: Permission hooks currently intercept shell commands only. File operations use Codex's built-in sandbox and exec policy. Check AGENTS.md in the project root for additional context.
...
- Codex uses AGENTS.md (not CLAUDE.md) for project-level instructions. Orchestratia context is injected via the SessionStart hook.
- The PreToolUse hook currently intercepts `Bash` tool calls only. File operations (`write_file`, `read_file`, etc.) are governed by Codex's sandbox mode and `.codexpolicy` rules.
- Your skill directory is at `~/.agents/skills/orchestratia/` (shared with Gemini CLI) or `~/.codex/skills/orchestratia/`.
- Hooks require `features.codex_hooks = true` in `~/.codex/config.toml` (set by the Orchestratia installer).
